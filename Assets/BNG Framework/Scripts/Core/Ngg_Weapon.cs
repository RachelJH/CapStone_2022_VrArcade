...
namespace BNG {
    ...
    /// </summary>
    public class Ngg_Weapon : GrabbableEvents {
        ...
        [Tooltip("Hit Effects change bullet spawned at point of impact")]
        public GameObject HitFXBulletPrefab;
        ...
        private void Update()
        {
            ...
            NetworkedGrabbable ng;
            GameObject playerC;
            ng = GetComponent<NetworkedGrabbable>();
            if (ng.BeingHeld && gameObject.layer != LayerMask.NameToLayer("GrabbableRocket")) {
                playerC = GameObject.Find("PlayerC");
                GameObject playerCanvas = playerC.transform.Find("PlayerController").transform.Find("Canvas").gameObject;
                playerCanvas.SetActive(false);
                gameObject.layer = LayerMask.NameToLayer("GrabbableRocket");
        ...
        public virtual void Shoot() {
            ...
            if (useProjectile) {
                ...
                //수정
                Debug.Log("instanti");
                //GameObject projectile = Instantiate(HitFXBulletPrefab, MuzzlePointTransform.position, MuzzlePointTransform.rotation) as GameObject;

                if (projectilePrefab != null) {
                    GameObject projectile = PhotonNetwork.Instantiate("projectilePrefab", MuzzlePointTransform.position, MuzzlePointTransform.rotation); 
                    //GameObject projectile = Instantiate(projectilePrefab, MuzzlePointTransform.position, MuzzlePointTransform.rotation);
                    }

                    GameObject flash = PhotonNetwork.Instantiate("MuzzleFlashObject", MuzzlePointTransform.position, MuzzlePointTransform.rotation);
                    //GameObject flash = Instantiate(MuzzleFlashObject, MuzzlePointTransform.transform);
            ...
            if (source != null)
            {
                ...
                else
                {
                    ...
                    if ((newAS = Instantiate(source)) != null && newAS.outputAudioMixerGroup != null && newAS.outputAudioMixerGroup.audioMixer != null)
                    {
                        ...
                        DestroyAfter(newAS.gameObject, 4);
        ...
        {
            Debug.Log("ReE?");
            VRUtils.Instance.PlaySpatialClipAt(ReloadClip, transform.position, GunShotVolume);
            //reloadSource.Play();
