...
namespace DynamicMeshCutter
{
    ...
    public static class MeshCreation
    {
        ...
        {
            GameObject tempP = Resources.Load<GameObject>(target.GameobjectRoot.name);
            parent = GameObject.Instantiate(tempP).transform;
            //포톤 수정?

            Debug.Log(target.GameobjectRoot.name);
            //if(Photon.Pun.PhotonNetwork.IsMasterClient)
                //parent = Photon.Pun.PhotonNetwork.Instantiate(target.GameobjectRoot.name, target.GameobjectRoot.transform.position, Quaternion.identity, 0).transform;
            parent.gameObject.tag = "DeadZombi";
        ...
        public static void TranslateCreatedObjects(Info info, GameObject[] createdObjects, MeshTarget[] targets, float separation)
        {
            ...
            int cLength = createdObjects.Length;
            if (createdObjects.Length >= 200) {
                cLength = 200;
            }
