...
namespace DynamicMeshCutter
{
    ...
    //수정 모노비해버펀
    public abstract class CutterBehaviour : MonoBehaviourPun
    {
                ...
                if (DestroyTargets)
                {
                    if (info.MeshTarget)
                    {
                        ...
                        if (info.MeshTarget.GameobjectRoot != null)
                            //PhotonNetwork.Destroy(info.MeshTarget.GameobjectRoot);
                        Destroy(info.MeshTarget.GameobjectRoot, 0);
                        else
                            //PhotonNetwork.Destroy(info.MeshTarget.gameObject);
