...
namespace VRBuilder.Animations.Behaviors
{
    ...
    public class FollowPathBehavior : Behavior<FollowPathBehavior.EntityData>
    {
        ...
        public FollowPathBehavior(ISceneObject target, IPathProperty path, float duration, AnimationCurve velocity, bool keepRelativePosition = false, bool keepRelativeRotation = false, bool reverse = false, bool pingPong = false, int repeats = 1, string name = "Follow Path") : this(ProcessReferenceUtils.GetNameFrom(target), ProcessReferenceUtils.GetNameFrom(path), duration, velocity, keepRelativePosition, keepRelativeRotation, reverse, pingPong, repeats, name)
        {
        }

        public FollowPathBehavior(string targetName, string pathName, float duration, AnimationCurve velocity, bool keepRelativePosition = false, bool keepRelativeRotation = false, bool reverse = false, bool pingPong = false, int repeats = 1, string name = "Follow Path")
        {
