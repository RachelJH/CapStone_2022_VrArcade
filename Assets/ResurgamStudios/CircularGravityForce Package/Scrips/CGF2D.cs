    ...
    public class CGF2D : MonoBehaviour
    {
        ...
        private void RigidbodieCollectionClean()
        {
            if (rigidbodyCollection == null)
                rigidbodyCollection = new Dictionary<Rigidbody2D, bool>();
        ...
        private void ApplyForce(Rigidbody2D rigid, Transform trans, Collider2D coll)
        {
            ...
            switch (_forceType2D)
            {
                ...
                case ForceType2D.GravitationalAttraction:
                    ...
                    break;
                case ForceType2D.Velocity:
                    ApplyCGFVelocity(rigid, transPos, trans);
                    break;
                case ForceType2D.AngularVelocity:
        ...
        private void ApplyCGFForceAtPosition(Rigidbody2D rigid, Vector3 transPos, Transform trans)
        {
            ...
            if (CounterGravity)
                rigid.AddForce(-(Physics.gravity * CounterGravityMultiplier) * rigid.gravityScale);
        }
        ...
        {
            Vector2 force = transPos;
            Vector3 velocity = new Vector3(rigid.velocity.x, rigid.velocity.y, 0f);

            if (_projectRight || _shape2D == Shape2D.Raycast)
            {
                force = ((trans.right) * ForcePower) - (VelocityDamping * velocity);
            }
            else
            {
                force = ((rigid.transform.position - transPos) * ForcePower) - (VelocityDamping * velocity);
            }
        ...
        {
            var torque = -(ForcePower + ((rigid.angularVelocity * AngularVelocityDamping) * Time.deltaTime));

            if (LerpVelocity)
                rigid.angularVelocity = Mathf.Lerp(rigid.angularVelocity, torque, Time.deltaTime);
            else
