...
namespace CircularGravityForce
{
    ...
    public class CGF2D_Editor : Editor
    {
        ...
        private SerializedProperty counterGravity_property;
        private SerializedProperty counterGravityMultiplier_property;
        private SerializedProperty lerpVelocity_property;
        ...
        public void OnEnable()
        {
            ...
            counterGravity_property = serializedObject.FindProperty("counterGravity");
            counterGravityMultiplier_property = serializedObject.FindProperty("counterGravityMultiplier");
            lerpVelocity_property = serializedObject.FindProperty("lerpVelocity");
        ...
        public override void OnInspectorGUI()
        {
            ...
            switch (forceType_property.enumValueIndex)
            {
                ...
                case (int)CGF2D.ForceType2D.Force:
                    helpText = "Adds force to the objects.";
                    break;
                case (int)CGF2D.ForceType2D.Torque:
                    helpText = "Adds torque to the objects.";
                    break;
                case (int)CGF2D.ForceType2D.GravitationalAttraction:
                    helpText = "Adds gravitational attraction based off newtonian gravity.";
                    break;
                case (int)CGF2D.ForceType2D.Velocity:
                    helpText = "Adds velocity to the objects.";
                    break;
                case (int)CGF2D.ForceType2D.AngularVelocity:
                    helpText = "Adds angular velocity to the objects.";
                    break;
            }

            if (forceType_property.enumValueIndex != (int)CGF2D.ForceType2D.Velocity &&
               forceType_property.enumValueIndex != (int)CGF2D.ForceType2D.AngularVelocity)
            {
                /*<----------------------------------------------------------------------------------------------------------*/
                EditorGUI.BeginChangeCheck();
            ...
            else
            {
                /*<----------------------------------------------------------------------------------------------------------*/
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(lerpVelocity_property, new GUIContent("Lerp Velocity"));
                if (EditorGUI.EndChangeCheck())
                {
                    lerpVelocity_property.boolValue = EditorGUILayout.Toggle(lerpVelocity_property.boolValue);
                    change = true;
                }
                /*<----------------------------------------------------------------------------------------------------------*/
            }
            ...
            switch (forceType_property.enumValueIndex)
            {
                ...
                case (int)CGF.ForceType.AngularVelocity:
                    /*<----------------------------------------------------------------------------------------------------------*/
                    EditorGUI.BeginChangeCheck();
            ...
            /*<----------------------------------------------------------------------------------------------------------*/
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(counterGravity_property, new GUIContent("Counter Gravity"));
            if (EditorGUI.EndChangeCheck())
            {
                counterGravity_property.boolValue = EditorGUILayout.Toggle(counterGravity_property.boolValue);
                change = true;
            }
            /*<----------------------------------------------------------------------------------------------------------*/

            if (counterGravity_property.boolValue)
            {
                /*<----------------------------------------------------------------------------------------------------------*/
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(counterGravityMultiplier_property, new GUIContent("Counter Gravity Multiplier"));
                if (EditorGUI.EndChangeCheck())
                {
                    counterGravityMultiplier_property.floatValue = EditorGUILayout.FloatField(counterGravityMultiplier_property.floatValue);
                    change = true;
                }
                /*<----------------------------------------------------------------------------------------------------------*/

                if (helpText != string.Empty)
                    EditorGUILayout.HelpBox("Keep in mind if CGFs are overlapping it will apply both counters. So recommend not overlapping CGFs unless that's the desired effect.", messageType, true);
            }
        ...
        void OnSceneGUI()
        {
            ...
            if (cgf._drawGravityProperties.DrawGizmo)
            {
                ...
                switch (cgf._shape2D)
                {
                    case CGF2D.Shape2D.Sphere:
                        ...
                        if ((cgf._forceType2D == CGF2D.ForceType2D.ForceAtPosition ||
                            ...
                            cgf._forceType2D == CGF2D.ForceType2D.Velocity ||
                        ...
                        else if (cgf._forceType2D == CGF2D.ForceType2D.Torque || cgf._forceType2D == CGF2D.ForceType2D.AngularVelocity)
                        {
                        ...
                        if (cgf._forceType2D != CGF2D.ForceType2D.Torque && cgf._forceType2D != CGF2D.ForceType2D.AngularVelocity)
                        {
                            if (cgf._forcePositionProperties.ForcePosition == CGF.ForcePosition.ThisTransform)
                            {
                ...
                if (counterGravity_property.boolValue)
                {
                    ...
                    centeredStyle.fixedWidth = 300f;
                    centeredStyle.normal.textColor = mainColor;
                    centeredStyle.fontStyle = FontStyle.Bold;
                    var labelPos = cgf.transform.position;
                    labelPos.y = cgf.transform.position.y + .5f;

                    Handles.Label(labelPos, string.Format("Gravity -= {0}", (Physics2D.gravity * cgf.CounterGravityMultiplier)), centeredStyle);
                }
            }
